...
public class EmpController extends BaseController {
    ...
    public AjaxResult businessList(@PathVariable String innerCode) {
        ...
        Emp emp = new Emp();
        emp.setRegionId(vendingMachine.getRegionId()); //设备所属的区域id
        emp.setRoleCode(DkdContants.ROLE_CODE_BUSINESS); // 角色编号：运营员
    ...
    public AjaxResult operatorList(@PathVariable String innerCode) {
        // 根据售货机编号查询售货机信息
        VendingMachine vendingMachine = vendingMachineService.selectVendingMachineByInnerCode(innerCode);
        if (vendingMachine == null) {
            return error("售货机不存在");
        ...
        Emp emp = new Emp();
        emp.setRegionId(vendingMachine.getRegionId()); //设备所属的区域id
        emp.setRoleCode(DkdContants.ROLE_CODE_OPERATOR); // 角色编号：运维员
        emp.setStatus(DkdContants.EMP_STATUS_NORMAL); // 员工启用
        return success(empService.selectEmpList(emp));
    }
}
